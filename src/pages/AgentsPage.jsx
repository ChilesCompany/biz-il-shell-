...
const AGENTS = [
  ...
  { name: 'weekly-review',       category: 'VP Sales',      status: 'ok',   last: 'Today 7:05 AM',  schedule: 'Weekdays 7AM ET' },
]
...
export default function AgentsPage() {
  ...
  return (
    <PageShell title="Agent Registry" description="All BIZ IL skills — click Run to execute any agent" icon={Bot} color="orange">
            ...
            {AGENTS.map(agent => (
              <tr key={agent.name} className="border-b border-border last:border-0 hover:bg-surface-2 transition-colors">
                <td className="px-4 py-3">
                  <span className="font-mono text-xs font-medium text-ink-2">{agent.name}</span>
                </td>
                <td className="px-4 py-3 hidden md:table-cell">
                  <span className={`stat-pill ${CATEGORY_COLOR[agent.category] ?? 'bg-surface-3 text-ink-3'}`}>
                    {agent.category}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-xs font-medium ${agent.status === 'ok' ? 'text-emerald-600' : 'text-ink-4'}`}>
                      {agent.status === 'ok' ? 'Active' : 'Idle'}
                    </span>
                  </div>
                </td>
                <td className="px-4 py-3 hidden lg:table-cell">
                  <span className="text-xs text-ink-3 flex items-center gap-1">
                    <Clock size={11} /> {agent.last}
                  </span>
                </td>
                <td className="px-4 py-3 hidden lg:table-cell">
                  <span className="text-xs text-ink-3">{agent.schedule}</span>
                </td>
