...
const SKILL_PROMPTS = {
  ...
  'enrich':              'Run the enrich skill. Tell me what prospect enrichment capabilities are available.',
    ...
    try {
      const res = await fetch('/api/claude', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: 'You are Commander BIZ, the AI operations layer for Biltmore Management Advisors. Be concise, structured, and actionable. Use markdown formatting.',
        ...
        <div className="p-5">
          {status === 'idle' && (
            <div className="text-center py-6">
              <div className="text-sm text-ink-2 mb-4">Run <code className="font-mono text-xs bg-surface-3 px-1.5 py-0.5 rounded">{agent.name}</code> via Commander BIZ?</div>
              <button onClick={runAgent} className="px-6 py-2.5 bg-orange text-white text-sm font-semibold rounded-lg hover:bg-orange-dark transition-colors shadow-orange">Run Agent</button>
            </div>
          )}
          {status === 'running' && (
          ...
          {(status === 'done' || status === 'error') && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                {status === 'done' ? <CheckCircle size={14} className="text-emerald-500" /> : <AlertCircle size={14} className="text-red-500" />}
                <span className={'text-xs font-semibold ' + (status === 'done' ? 'text-emerald-600' : 'text-red-600')}>{status === 'done' ? 'Completed' : 'Failed'}</span>
              </div>
              <div ref={outputRef} className="bg-surface-2 border border-border rounded-xl p-4 text-xs text-ink-2 leading-relaxed whitespace-pre-wrap max-h-64 overflow-y-auto font-mono">{output}</div>
              <div className="flex gap-2 mt-4">
                <button onClick={runAgent} className="flex-1 py-2 border border-border text-xs font-medium text-ink-2 hover:border-orange/40 hover:text-orange rounded-lg transition-all">Run Again</button>
                <button onClick={onClose} className="flex-1 py-2 bg-navy text-white text-xs font-semibold rounded-lg hover:bg-navy-mid transition-colors">Close</button>
              </div>
            </div>
          )}
        </div>
